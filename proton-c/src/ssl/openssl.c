...
static int init_ssl_socket(pn_transport_t *, pn_ssl_t *);
...
int pn_ssl_init( pn_transport_t *transport, pn_ssl_domain_t *domain, const char *session_id)
{
    pn_ssl_t *ssl = transport->ssl;
...
{
  return ssl && ssl->domain && ssl->domain->allow_unsecured;
}
...
static int init_ssl_socket(pn_transport_t* transport, pn_ssl_t *ssl)
{
  if (ssl->ssl) return 0;
