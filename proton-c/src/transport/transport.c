...
static void pn_io_layer_setup(pn_transport_t *transport, unsigned int l)
{
  assert(l==0);
  ...
  if (transport->server)
  {
    ...
    if (transport->ssl && pn_ssl_allow_unsecured(transport->ssl)) {
...
ssize_t pn_io_layer_input_autodetect(pn_transport_t *transport, unsigned int layer, const char *bytes, size_t available)
{
  ...
  case PNI_PROTOCOL_AMQP1:
    if (transport->sasl && pn_sasl_state(transport->sasl)==PN_SASL_IDLE) {
      pn_transport_log(transport, "AMQP detected without SASL before it");
      if (pn_sasl_skipping_allowed(transport->sasl)) {
        pn_sasl_done(transport->sasl, PN_SASL_SKIPPED);
      } else {
