...
class Transport(object):
  ...
  def __init__(self, mode=None, _trans=None):
    if not mode and not _trans:
      # XXX: Need to figure out if we are client or server side
      self._trans = pn_transport_client()
    elif not mode:
      ...
      self._trans = pn_transport_server()
    else:
...
class SSL(object):
  ...
  def __new__(cls, transport, domain, session_details=None):
    ...
    if transport._ssl:
      ...
      if obj._ssl is None:
        raise SSLUnavailable()
      pn_ssl_init( transport._trans, domain._domain, session_id )
      transport._ssl = obj
